...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    /**
     * 查询 userA 和 userB 之间的聊天记录，按时间升序排序
     */
    public List<Message> getChatHistoryBetweenUsers(String usernameA, String usernameB) {
        List<Message> messages = new ArrayList<>();
        SQLiteDatabase db = this.getReadableDatabase();

        long userAId = getUserIdByUsername(usernameA);
        long userBId = getUserIdByUsername(usernameB);

        if (userAId == -1 || userBId == -1) {
            return messages; // 返回空列表而不是 null
        }
        ...
        String[] args = {
                String.valueOf(userAId), String.valueOf(userBId),
                String.valueOf(userBId), String.valueOf(userAId)
        };
        ...
        if (cursor != null) {
            while (cursor.moveToNext()) {
                ...
                long senderId = cursor.getLong(cursor.getColumnIndexOrThrow(COLUMN_SENDER_ID));
                long receiverId = cursor.getLong(cursor.getColumnIndexOrThrow(COLUMN_RECEIVER_ID));
                String content = cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_MESSAGE));
                ...
                messages.add(new Message(id, senderUsername, receiverUsername, content, timestamp));
            }
            cursor.close();
        }
